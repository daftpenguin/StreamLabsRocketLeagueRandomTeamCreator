...
global TrollEnabled
TrollEnabled = False
global TrollLastTime
TrollLastTime = 0
global TrollMessages
TrollMessages = [
    "Tri's forehead can be seen from space",
    "If Tri played on fullscreen he wouldn't lose to golds in ones",
    "Tri got boosted to champ by randos",
    "Tri used BakkesMod to hack his dribble challenge record",
    "TrashHouse",
    "Tri paid for partner",
    ""
]
...
def Execute(data):
    ...
    # Trolling
    if data.IsChatMessage() and (data.User.lower() not in ["trihouse", "tri_house_bot"]):
        if TROLL_MSG_TIMER_MINS > 0 and TrollEnabled:
            if TrollLastTime == 0:
                TrollLastTime = time.time()
            if (time.time() - TrollLastTime) >= (TROLL_MSG_TIMER_MINS * 60):
                TrollLastTime = time.time()
                r = randint(0, len(TrollMessages))
                if TrollMessages[r] is not None:
                    Parent.SendStreamMessage(TrollMessages[r])

    if data.IsChatMessage() and data.User.lower() == "daftpenguin" and data.GetParam(0).lower() == "!daft":
    ...
    if data.IsChatMessage() and data.GetParam(0).lower() == ScriptSettings.RegisterCommand:
        ...
        if len(params) == 0:
            ...
            if mmr < 0:
                ...
                Parent.SendStreamMessage("@{0} quit trolling u bish".format(data.User))
            else:
